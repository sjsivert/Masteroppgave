...
with description("main Local Univariate LSTM integration test", "this") as self:
    ...
    with before.each:
        ...
        when(self.mocked_pipeline).run().thenReturn((mock(DataLoader), mock(DataLoader), mock(DataLoader), mock(MinMaxScaler)))
    ...
    with _it("can continue the last ran experiment from disk"):
    ...
    with _it("can load a previously saved experiment"):
    ...
    with _it("can run an experiment with local_univariate_arima"):
        # Arrange
        unstub()
        model_options = {
