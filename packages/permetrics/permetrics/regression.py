...
from numpy import max, round, sqrt, abs, mean, dot, divide, arctan, sum, any, median, log, var, std, maximum, roll
...
class Metrics:
    ...
    def __init__(self, y_true=None, y_pred=None):
        ...
        if (y_true is not None) and (y_pred is not None):
            self.y_true, self.y_pred, self.y_true_clean, self.y_pred_clean, self.onedim = self.__clean_data__(y_true,
                                                                                                              y_pred)
    ...
    def __clean_data__(self, y_true=None, y_pred=None):
        ...
        if type(y_true) is ndarray and type(y_pred) is ndarray:
            ...
            y_pred = y_pred.astype(
                'float64')  # x = x[~np.isnan(x)] can't remove if array is dtype object, only work with dtype float
    ...
    def symmetric_mean_absolute_percentage_error_custom(self, clean=True, multi_output="raw_values", decimal=3,
                                                        y_true_custom=None, y_pred_custom=None, **kwargs):
        """
            Symmetric Mean Absolute Percentage Error: Good if mape < 20%. Smaller is better
        """
        epsilon = 0.1
        y_true, y_pred, onedim = self.__get_data__(clean, kwargs)
        if y_true_custom is not None and y_pred_custom is not None:
            y_true = y_true_custom
            y_pred = y_pred_custom

        if onedim:
            denominator = maximum(abs(y_true) + abs(y_pred) + epsilon, 0.5 + epsilon)
            return round(
                mean(2 * abs(y_pred - y_true) / denominator),
                decimal)
        else:
            denominator = maximum(abs(y_true) + abs(y_pred) + epsilon, 0.5 + epsilon)

            temp = mean(2 * abs(y_pred - y_true) / denominator, axis=0)
            return self.__multi_output_result__(temp, multi_output, decimal)

    def owa(self, clean=False, multi_output="raw_values", decimal=3, **kwargs):

        y_true, y_pred, onedim = self.__get_data__(clean, kwargs)
        y_naive = roll(y_true, 1)
        y_naive[0] = y_true[0]

        temp_mase = self.mase(y_true_custom=y_true, y_pred_custom=y_pred, clean=clean, multi_output=multi_output, decimal=decimal, **kwargs)
        temp_mase_naive = self.mase(y_true_custom=y_true, y_pred_custom=y_naive, clean=clean, multi_output=multi_output, decimal=decimal, **kwargs)

        temp_smape = self.smape_custom(y_true_custom=y_true, y_pred_custom=y_pred, clean=clean, multi_output=multi_output, decimal=decimal, **kwargs)
        temp_smape_naive = self.smape_custom(y_true_custom=y_true, y_pred_custom=y_naive, clean=clean, multi_output=multi_output, decimal=decimal, **kwargs)

        owa = 1/2*(temp_mase/temp_mase_naive + temp_smape/temp_smape_naive)
        return self.__multi_output_result__(owa, multi_output, decimal)
    ...
    def mean_absolute_scaled_error(self, m=1, clean=True, multi_output="raw_values", decimal=3, y_true_custom=None,
                                   y_pred_custom=None, **kwargs):
        """
    ...
    def confidence_index(self, clean=False, multi_output="raw_values", decimal=3, **kwargs):
        ...
        """
        y_true, y_pred, onedim = self.__get_data__(clean, kwargs)
        r = self.pearson_correlation_index(clean=clean, multi_output="raw_values", decimal=decimal, y_true=y_true,
                                           y_pred=y_pred)
    ...
    def gini_coefficient(self, clean=False, multi_output="raw_values", decimal=3, **kwargs):
        ...
        if onedim:
            ...
            population_delta = 1.0 / len(y_true)
            accumulated_population_percentage_sum, accumulated_loss_percentage_sum, score = zeros(col), zeros(
                col), zeros(col)
    ...
    SMAPECustom = smape_custom = symmetric_mean_absolute_percentage_error_custom
